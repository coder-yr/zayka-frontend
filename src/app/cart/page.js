...
import { Search, ShoppingCart, Bell, Trash2, Plus, Minus, Truck, CreditCard } from "lucide-react";
import { useTranslation } from "react-i18next";
import { useCart } from "@/context/CartContext";
import LanguageSwitcher from "@/app/components/common/LanguageSwitcher";

const getLocalizedTitle = (item, language) => {
...
export default function CartPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FBFBFB] font-sans selection:bg-[#C82333] selection:text-white rtl-text-start">
      <nav className="w-full bg-white sticky top-0 z-50 py-4 px-6 md:px-12 border-b border-gray-100/50">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="font-bold text-gray-900 text-lg md:text-xl tracking-tight leading-none">Zayka POS</span>
          </div>

          <div className="hidden md:flex items-center gap-8 text-[15px] font-medium text-gray-500">
            <Link href="/home" className="hover:text-gray-900 transition-colors">{t("menu")}</Link>
          ...
          <div className="flex items-center gap-5">
            ...
            <button className="text-gray-600 hover:text-gray-900" type="button" aria-label="Notifications">
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-12 py-10 lg:py-16">
        <div className="mb-10 relative inline-block">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 tracking-tight">
            {t("your_cart_items", { count: cartTotalItems })}
          </h1>
          <div className="absolute -bottom-2.5 left-0 w-16 h-[4px] bg-[#D92B2B] rounded-full" />
        </div>

        <div className="cart-main-layout flex flex-col lg:flex-row gap-8 lg:gap-12">
          <div className="flex-grow w-full lg:w-[65%] space-y-6">
            {cartItems.length === 0 ? (
              <div className="bg-white p-12 rounded-[2rem] shadow-sm border border-gray-50 flex flex-col items-center justify-center text-center">
                ...
                <h3 className="text-xl font-bold text-gray-900 mb-2">{t("your_cart_empty")}</h3>
                <p className="text-gray-500 mb-6 max-w-sm">{t("select_items_prompt")}</p>
              ...
              cartItems.map((item) => {
                const title = getLocalizedTitle(item, i18n.language);
                return (
                  <div key={item.id} className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-50 flex flex-col sm:flex-row gap-6 relative group">
                    ...
                    <div className="flex flex-col flex-grow">
                      <div className="mb-1">
                        <span className={`px-2.5 py-0.5 rounded text-[10px] font-bold tracking-wider ${item.badgeColor || "bg-gray-100 text-gray-600"}`}>
                          {item.badgeText}
                        </span>
                      </div>

                      <h3 className="text-lg font-bold text-gray-900 mb-2 leading-tight pr-6">{title}</h3>
                      <p className="text-[13px] text-gray-500 leading-relaxed mb-6 max-w-sm">{item.description}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
              <div className="bg-[#FFEBE5] rounded-3xl p-6 md:p-8 relative overflow-hidden flex flex-col justify-center border border-orange-50/50">
                <h4 className="text-[17px] font-bold text-[#802D2D] mb-1.5 leading-tight">{t("complimentary_side")}</h4>
                <p className="text-[14px] text-[#A64646] mb-4 pr-10 leading-snug font-medium">{t("add_signature_side")}</p>
                <button className="text-[#C82333] font-bold text-[13px] hover:text-red-800 transition-colors underline underline-offset-4 w-max" type="button">
                  {t("add_to_order")}
                </button>
              ...
              <div className="bg-[#F6F6F6] rounded-3xl p-6 md:p-8 flex items-center gap-5 border border-gray-100">
                ...
                <div>
                  <h4 className="text-[15px] font-bold text-gray-900 mb-1">{t("fast_delivery")}</h4>
                  <p className="text-[13px] text-gray-500 font-medium tracking-tight">{t("eta_25_35")}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="w-full lg:w-[35%] flex flex-col gap-6">
            <div className="cart-summary-sticky bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100 sticky top-28">
              <h2 className="text-2xl font-bold text-gray-900 mb-8">{t("order_summary")}</h2>

              <div className="space-y-5 mb-8">
                {cartItems.map((item) => (
                  <div key={item.id} className="flex justify-between text-[15px]">
                    <span className="text-gray-500 truncate pr-4">
                      {getLocalizedTitle(item, i18n.language)} <span className="text-gray-400 mx-0.5">x</span> {item.quantity}
                    </span>
              ...
              <div className="flex items-center gap-2 mb-8 p-1.5 bg-[#F6F6F6] rounded-xl border border-gray-100">
                ...
                />
                <button className="bg-[#1C1C1C] text-white px-6 py-2.5 rounded-lg text-sm font-semibold hover:bg-black transition-colors shadow-sm" type="button">
                  {t("apply")}
                </button>
              </div>
            ...
            <div className="bg-[#FAFAFA] rounded-2xl p-6 border border-gray-100">
              <div className="flex items-center gap-2 mb-3 text-gray-500">
                ...
                <span className="text-[11px] font-bold tracking-widest uppercase">{t("delivery_to")}</span>
              </div>
              <p className="text-[14px] font-bold text-gray-900 mb-3 leading-tight">124 Curator Avenue, Upper East Side</p>
              <button className="text-[10px] font-bold text-[#D92B2B] uppercase tracking-wide hover:underline underline-offset-2" type="button">
                {t("change_address")}
              </button>
            </div>
          </div>
      ...
      <footer className="w-full bg-[#fafafa] py-10 mt-10 border-t border-gray-100">
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex flex-col md:flex-row items-center justify-between text-xs text-gray-500 font-medium">
          <div className="font-bold text-gray-900 text-sm mb-4 md:mb-0">Zayka POS</div>
          <div className="flex gap-6 mb-4 md:mb-0">
            <Link href="#" className="hover:text-gray-900 transition-colors">{t("privacy_policy")}</Link>
            <Link href="#" className="hover:text-gray-900 transition-colors">{t("terms_service")}</Link>
            <Link href="#" className="hover:text-gray-900 transition-colors">{t("accessibility")}</Link>
            <Link href="#" className="hover:text-gray-900 transition-colors">{t("contact_us")}</Link>
          </div>
          <p>© 2024 Zayka POS. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
